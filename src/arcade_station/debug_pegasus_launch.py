...
def scan_for_pegasus_binary():
    ...
    if not names_to_search:
        log_message(
            f"Unknown OS type: {os_type}, using all known binary names", "DEBUG"
        )
        names_to_search = [
            name for sublist in binary_names.values() for name in sublist
        ]
    ...
    locations_to_search = [
        ...
        ),  # Parent of Python executable directory
...
def try_launch_pegasus(binary_path):
    ...
    try:
        ...
        if platform.system() == "Windows":
            ...
            process = subprocess.Popen([binary_path], cwd=working_dir)
        ...
        if process.poll() is None:
            ...
            log_message(
                f"Process exited immediately with code: {process.returncode}", "DEBUG"
            )
            return False
...
def repair_pegasus_configuration():
    ...
    try:
        ...
        if not binaries:
            log_message(
                "No Pegasus binaries found, cannot repair configuration", "DEBUG"
            )
            return False
        ...
        try:
            ...
            if os_type == "Windows" and binary_name.endswith(".exe"):
                if (
                    "pegasus" in installed_games
                    and "windows_binary" in installed_games["pegasus"]
                ):
                    if installed_games["pegasus"]["windows_binary"] != binary_name:
                        log_message(
                            f"Updating Windows binary name to: {binary_name}", "DEBUG"
                        )
                ...
                if (
                    "pegasus" in installed_games
                    and "mac_binary" in installed_games["pegasus"]
                ):
                    if installed_games["pegasus"]["mac_binary"] != binary_name:
                        log_message(
                            f"Mac binary name should be updated to: {binary_name}",
                            "DEBUG",
                        )
                        updated_config = True
            elif os_type == "Linux":
                if (
                    "pegasus" in installed_games
                    and "linux_binary" in installed_games["pegasus"]
                ):
                    if installed_games["pegasus"]["linux_binary"] != binary_name:
                        log_message(
                            f"Linux binary name should be updated to: {binary_name}",
                            "DEBUG",
                        )
                        updated_config = True
            ...
            if updated_config:
                ...
                log_message(
                    f"The pegasus-fe directory should be: {binary_dir}", "DEBUG"
                )
            else:
...
def main():
    ...
    parser.add_argument(
        "--scan-only",
        action="store_true",
        help="Only scan for Pegasus binaries without trying to launch",
    )
    parser.add_argument(
        "--repair", action="store_true", help="Attempt to repair Pegasus configuration"
    )
    parser.add_argument(
        "--create-launcher",
        action="store_true",
        help="Create a direct launcher script for Pegasus",
    )
    ...
    log_message(
        f"Script directory: {os.path.dirname(os.path.abspath(__file__))}", "DEBUG"
    )
    ...
    if args.scan_only:
        log_message(
            "Scan complete. Not attempting to launch (--scan-only specified).", "DEBUG"
        )
        return
