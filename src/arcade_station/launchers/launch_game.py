...
def launch_game(game_name):
    ...
    if game_name in config["games"]:
        ...
        if isinstance(game_config, dict) and "rom" in game_config:
            ...
            log_message(
                f"Launching MAME game - ROM: {rom}, State: {state}", "GAME_LAUNCH"
            )
            mame_script = os.path.abspath(
                os.path.join(
            ...
            if process:
                ...
                log_message(
                    f"Failed to get process handle for MAME game: {rom}", "GAME_LAUNCH"
                )
            ...
            if game_path and os.path.exists(game_path):
                ...
                try:
                    ...
                    if platform.system() == "Windows":
                        ...
                        success = start_process_with_powershell(
                            file_path=game_path, working_dir=game_dir
                        )

                        if not success:
                            log_message(
                                "Failed to start game with PowerShell", "GAME_LAUNCH"
                            )
                        ...
                        set_process_priority(process.pid, "high")
                        log_message(
                            f"Successfully launched game via subprocess: {game_path}",
                            "GAME_LAUNCH",
                        )
                ...
                log_message(
                    f"Game path not found or invalid: {game_path}", "GAME_LAUNCH"
                )
    else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        log_message(
            "No game name provided. Usage: python launch_game.py <game_name>", "GAME"
        )
