...
def ensure_required_packages():
    ...
    try:
        ...
        log_message(
            "Installing required Windows packages for focus management...", "BANNER"
        )
        try:
            subprocess.check_call(
                [sys.executable, "-m", "pip", "install", "pywin32", "psutil"]
            )
...
def monitor_itgmania_log(config_path="display_config.toml"):
    ...
    try:
        ...
        if not log_file_path:
            log_message(
                "ITGMania log file path not specified in configuration", "BANNER"
            )
            return
        ...
        if not log_file.parent.exists():
            log_message(
                f"Directory for ITGMania log file does not exist: {log_file.parent}",
                "BANNER",
            )
            log_message(
                f"Will attempt to create directory: {log_file.parent}", "BANNER"
            )
            try:
        ...
        while True:
            try:
                ...
                if log_file.exists():
                    ...
                    log_message(
                        f"ITGMania log file no longer exists: {log_file}", "BANNER"
                    )
                    log_message("Attempting to recreate the file", "BANNER")
                    try:
                        log_file.touch()
                        log_message(
                            f"Recreated ITGMania log file: {log_file}", "BANNER"
                        )
                        last_mod_time = log_file.stat().st_mtime
                    except Exception as e:
...
def update_marquee_from_file(file_path, config):
    ...
    try:
        ...
        itgmania_base_path = config.get("dynamic_marquee", {}).get(
            "itgmania_base_path", ""
        )
        ...
        if event_match and banner_match:
            ...
            log_message(
                f"Found event: {event_type} with banner: {banner_path}", "BANNER"
            )

            # Process relative paths if needed
            if (
                banner_path.startswith("/") or banner_path.startswith("\\")
            ) and not banner_path[1:2] == ":":
