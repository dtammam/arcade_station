    ...
    for hotkey, action in key_mappings.items():
        ...
        if action == "kill_processes":
            script_path = os.path.join(
                os.path.dirname(__file__), "kill_all_and_reset_pegasus.py"
            )
            keyboard.add_hotkey(hotkey, lambda: launch_script(script_path))
        else:
            keyboard.add_hotkey(
                hotkey, lambda action_path=action_path: start_app(action_path)
            )
...
def kill_processes_from_toml(toml_file_path):
    ...
    if platform.system() == "Windows":
        processes_to_kill = [
            proc if proc.endswith(".exe") else f"{proc}.exe"
...
def get_pegasus_binary(installed_games):
    ...
    potential_paths = [
        ...
        os.path.abspath(
            os.path.join(os.path.dirname(__file__), "../../../../pegasus-fe")
        ),
    ...
    if not pegasus_base_path:
        ...
        pegasus_base_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "../../../pegasus-fe")
        )
    ...
    if os_type == "Windows":
        binary_path = os.path.join(
            pegasus_base_path, installed_games["pegasus"]["windows_binary"]
        )
    elif os_type == "Darwin":
        binary_path = os.path.join(
            pegasus_base_path, installed_games["pegasus"]["mac_binary"]
        )
    else:
        binary_path = os.path.join(
            pegasus_base_path, installed_games["pegasus"]["linux_binary"]
        )
    ...
    try:
        ...
        if not os.path.exists(pegasus_binary):
            ...
            possible_locations = [
                ...
                ),  # Up one level from current file
                os.path.abspath(
                    os.path.join(os.path.dirname(__file__), "..", "..")
                ),  # Up two levels
                os.path.abspath(
                ...
                for root, dirs, files in os.walk(location):
                    ...
                    if "pegasus-fe" in dirs:
                        ...
                        if os.path.exists(possible_binary):
                            log_message(
                                f"Found binary in pegasus-fe dir: {possible_binary}",
                                "GAME",
                            )
            ...
            if not os.path.exists(pegasus_binary):
                log_message(
                    "Failed to find Pegasus binary after extensive search", "GAME"
                )
                return False
        ...
        process = subprocess.Popen(pegasus_binary, shell=True, cwd=working_dir)
    ...
    try:
        ...
        if executable_path.endswith(".ps1"):
            if os_type == "Windows":
                ...
                log_message(
                    "PowerShell scripts are not supported on non-Windows systems.",
                    "MENU",
                )
        ...
        # Check if the file is a Python script
            ...
            if os_type == "Windows":
                ...
                log_message(
                    f"Launching Windows executable with shell=True: {executable_path}",
                    "MENU_DEBUG",
                )
...
def kill_process_by_identifier(identifier):
    ...
    identifier_patterns = {
        ...
        ],  # For backward compatibility, point to new ID
    ...
    for proc in psutil.process_iter(attrs=["pid", "name", "cmdline"]):
        try:
            ...
            if cmdline and any(
                any(pattern in arg for arg in cmdline) for pattern in patterns
            ):
...
def launch_script(script_path, identifier=None, extra_args=None):
    ...
    process = subprocess.Popen(
        args, creationflags=creationflags, startupinfo=startupinfo
    )
    return process
...
def start_process_with_powershell(file_path, working_dir=None, arguments=None):
    ...
    ps_module_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "windows", "core_functions.psm1")
    )
...
def log_message(message, prefix=""):
    ...
    try:
        ...
        if "log_file_path" in globals() and os.path.exists(
