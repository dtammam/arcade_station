...
def main():
    ...
    if not pegasus_started:
        log_message(
            "Failed to start Pegasus with primary method, trying alternatives", "RESET"
        )

        # Alternative 1: Try to find and run Pegasus using installed_games.toml
        try:
            log_message(
                "Trying to locate Pegasus executable from configuration", "RESET"
            )
            ...
            if os_type == "nt":  # Windows
                binary_name = config.get("pegasus", {}).get(
            ...
            if pegasus_path:
                ...
                log_message(
                    f"Attempting to launch Pegasus from: {pegasus_path}", "RESET"
                )
                ...
                if process.poll() is None:
                    log_message(
                        "Pegasus successfully started with alternative method", "RESET"
                    )
                else:
                    log_message(
                ...
                log_message(
                    "Could not find Pegasus executable in common locations", "RESET"
                )
