...
def connect_vpn():
    ...
    log_message(
        f"Initiating VPN [{app_name}] using config [{config_profile}]...", "VPN"
    )
    ...
    try:
        ...
        log_message(
            f"Waiting for {seconds_to_wait} seconds before checking VPN process...",
            "VPN",
        )
        ...
        log_message(
            f"Validated that the VPN process [{process_name}] is running!", "VPN"
        )
        return 0
