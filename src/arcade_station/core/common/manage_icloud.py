...
def clean_directory(directory, delete_files=True):
    ...
    if not delete_files:
        log_message(
            f"File deletion disabled, skipping cleanup of {directory}", "ICLOUD"
        )
        return 0
...
def icloud_manager():
    ...
    apple_services_path = icloud_config.get(
        "apple_services_path",
        r"C:\Program Files (x86)\Common Files\Apple\Internet Services",
    )
    processes_to_restart = icloud_config.get(
        "processes_to_restart", ["iCloudServices", "iCloudPhotos"]
    )
    upload_directory = icloud_config.get(
        "upload_directory", r"C:\Users\me\Pictures\Uploads"
    )
    ...
    if interval_seconds < minimum_interval:
        log_message(
            f"Interval too short ({interval_seconds}s), setting to {minimum_interval}s",
            "ICLOUD",
        )
    ...
    while True:
        try:
            ...
            log_message(
                f"Waiting {wait_time} seconds before cleaning directory", "ICLOUD"
            )
