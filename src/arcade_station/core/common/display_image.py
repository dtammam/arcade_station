...
class ImageWindow(QMainWindow):
    ...
    def __init__(self, image_path, background_color="black", screen_geometry=None):
        ...
        self.setGeometry(
            screen_geometry.x(), screen_geometry.y(), screen_width, screen_height
        )
...
def display_image_from_config(
    config_path="display_config.toml", close_event=None, use_default=False
):
    """
    ...
    if use_default:
        image_path = config["display"].get(
            "default_image_path", config["display"]["image_path"]
        )
    else:
