...
def take_screenshot_from_config():
    ...
    file_name = config["screenshot"]["file_name"] or datetime.now().strftime(
        "%Y-%m-%d %H-%M-%S"
    )
...
def take_screenshot(
    monitor_index=0, file_location=".", file_name=None, quality="High", config=None
):
    """
    ...
    if not screens:
        log_message(
            "No screens detected. Ensure you are running in a GUI-capable environment.",
            "SCREENSHOT",
        )
        return
    ...
    sound_file = os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), config["screenshot"].get("sound_file", "")
        )
    )
    ...
    if (
        ...
        and config["screenshot"].get("sound_file", "").strip()
    ):
        try:
