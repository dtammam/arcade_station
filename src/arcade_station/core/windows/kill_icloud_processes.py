...
def kill_icloud_processes():
    ...
    processes_to_restart = config.get("icloud_upload", {}).get(
            ...
            if "manage_icloud.py" in line:
                try:
                    ...
                    if parts:
                        pid = parts[-1]
                        log_message(
                            f"Killing iCloud manager process with PID: {pid}",
                            "ICLOUD_KILL",
                        )
                        subprocess.run(
                            ["taskkill", "/F", "/PID", pid],
    ...
    for service in services:
        try:
            ...
            if os.path.exists(service_path):
                ...
                log_message(
                    f"Service executable not found: {service_path}", "ICLOUD_KILL"
                )
        except Exception as e:
