    ...
    try:
        ...
        if os_type == "Windows":
            ...
            if not os.path.exists(activate_script):
                log_message(
                    f"Activation script not found at {activate_script}", "STARTUP"
                )
                return False

            # Set environment variables for activation
            os.environ["PATH"] = (
                os.path.join(venv_path, "Scripts") + os.pathsep + os.environ["PATH"]
            )
            ...
            # Set environment variables for activation
            os.environ["PATH"] = (
                os.path.join(venv_path, "bin") + os.pathsep + os.environ["PATH"]
            )
            os.environ["VIRTUAL_ENV"] = venv_path
...
def prepare_system():
    ...
    try:
        log_message(
            "Preparing system by killing processes and resetting state...", "STARTUP"
        )
...
def start_conditional_scripts():
    ...
    try:
        ...
        if vpn_enabled:
            ...
            log_message(
                f"Launched VPN connection script with PID: {vpn_process.pid}", "STARTUP"
            )
        ...
        dynamic_marquee_enabled = display_config.get("dynamic_marquee", {}).get(
            "enabled", False
        )
        itgmania_display_enabled = display_config.get("dynamic_marquee", {}).get(
            "itgmania_display_enabled", False
        )
        ...
        if dynamic_marquee_enabled and itgmania_display_enabled:
            ...
            itgmania_process = launch_script(
                itgmania_script, identifier="monitor_itgmania"
            )
            log_message(
                f"Launched ITGMania monitor script with PID: {itgmania_process.pid}",
                "STARTUP",
            )

        # iCloud upload management configuration (Windows-only)
        screenshot_config = load_toml_config("screenshot_config.toml")
        icloud_enabled = screenshot_config.get("icloud_upload", {}).get(
            "enabled", False
        )
        ...
        if icloud_enabled and determine_operating_system() == "Windows":
            ...
            try:
                ...
                icloud_script = os.path.join(
                    base_dir, "core", "common", "manage_icloud.py"
                )
                icloud_process = launch_script(
                    icloud_script, identifier="manage_icloud"
                )
                log_message(
                    f"Launched iCloud manager with PID: {icloud_process.pid}", "STARTUP"
                )
            except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Start Arcade Station frontend applications"
    )
    parser.add_argument(
        "--shell-mode", action="store_true", help="Run in shell replacement mode"
    )
    ...
    if not venv_setup:
        log_message(
            "Warning: Virtual environment setup failed, continuing with system Python",
            "STARTUP",
        )
    ...
    log_message(
        f"Launched kill_all_and_reset_pegasus.py with PID: {reset_process.pid}", "RESET"
    )

    # If running in shell replacement mode, we need to keep this process running
    if args.shell_mode:
        log_message(
            "Running in shell replacement mode, keeping process alive", "STARTUP"
        )
        try:
