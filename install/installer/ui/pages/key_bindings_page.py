...
class HyperlinkLabel(ttk.Label):
    ...
    def __init__(self, master, text, url, **kwargs):
        ...
        self.bind(
            "<Enter>", lambda e: self.configure(font=("Segoe UI", 9, "underline"))
        )
...
class KeyBindingsPage(BasePage):
    ...
    def __init__(self, container, app):
        ...
        self.set_title(
            "Key Bindings Setup", "Configure global hotkeys and manage processes"
        )
    ...
    def _setup_key_bindings_tab(self):
        ...
        ttk.Label(
            link_frame, text="Key syntax is from the Keyboard library.", justify="left"
        ).pack(side="left")
        ...
        keybindings_frame = ttk.LabelFrame(
            self.key_bindings_tab, text="Key bindings", padding=(10, 5)
        )
        ...
        ttk.Label(header_frame, text="Function", font=("Segoe UI", 9, "bold")).grid(
            row=0, column=0, sticky="w"
        )

        ttk.Label(header_frame, text="Script Path", font=("Segoe UI", 9, "bold")).grid(
            row=0, column=1, sticky="w"
        )

        ttk.Label(header_frame, text="Hotkey", font=("Segoe UI", 9, "bold")).grid(
            row=0, column=2, sticky="w"
        )

        # Separator
        ttk.Separator(keybindings_frame, orient="horizontal").pack(
        ...
        self.scrollable_frame.bind(
            "<Configure>",
            lambda e: canvas.configure(scrollregion=canvas.bbox("all"), width=500),
        )
        ...
        default_bindings = [
            (
                "Reset back to menu",
                "../arcade_station/core/common/kill_all_and_reset_pegasus.py",
                "ctrl+space",
            ),
            (
                "Kill all processes",
                "../arcade_station/core/common/kill_all.py",
                "ctrl+f9",
            ),
            (
                "Kill Arcade Station",
                "../arcade_station/core/common/kill_station_wrapper.py",
                "ctrl+f10",
            ),
            (
                "Take screenshot",
                "../arcade_station/core/common/monitor_screenshot.py",
                "+",
            ),
            (
                "Take screenshot",
            ...
            (
                "Start streaming",
                "../arcade_station/core/common/start_streaming.py",
                "ctrl+f4",
            ),
            (
                "Setup kiosk mode (Windows)",
                "../arcade_station/core/windows/setup_kiosk_mode.ps1",
                "ctrl+f3",
            ),
            (
                "Restore PC mode (Windows)",
                "../arcade_station/core/windows/restore_pc_mode.ps1",
                "ctrl+f2",
            ),
            (
                "Restart computer (Windows)",
                "../arcade_station/core/windows/restart_computer.ps1",
                "ctrl+`",
            ),
        ...
        if (
            key_bindings
            and "key_mappings" in key_bindings
            and self.custom_key_bindings_loaded
        ):
            ...
            for key, path in key_bindings["key_mappings"].items():
                ...
                self._add_key_binding_row(
                    self.scrollable_frame, i, display_name, path, key
                )
                i += 1
            ...
            for i, (display_name, script_path, key) in enumerate(default_bindings):
                self._add_key_binding_row(
                    self.scrollable_frame, i, display_name, script_path, key
                )
    ...
    def _add_key_binding_row(self, parent, index, display_name, script_path, key):
        ...
        display_entry = ttk.Entry(row_frame, textvariable=display_var, width=25)
        display_entry.grid(row=0, column=0, sticky="w")

        # Script path field
        script_var = tk.StringVar(value=script_path)
        script_entry = ttk.Entry(row_frame, textvariable=script_var, width=33)
        script_entry.grid(row=0, column=1, sticky="w")

        # Key field
        key_var = tk.StringVar(value=key)
        key_entry = ttk.Entry(row_frame, textvariable=key_var, width=12)
    ...
    def _setup_process_tab(self):
        ...
        process_frame = ttk.LabelFrame(
            self.process_tab, text="Processes to Kill", padding=(10, 5)
        )
...
marquee_image.exe"""
        ...
        self.processes_text = tk.Text(
            text_frame, height=10, width=50, yscrollcommand=scrollbar.set
        )
    ...
    def save_data(self):
        ...
        for display_var, script_var, key_var, row_frame in self.key_bindings:
            ...
            if (
                display_var is None
                or script_var is None
                or key_var is None
                or row_frame is None
            ):
        ...
        key_config = {"key_mappings": key_mappings}
        ...
        process_config = {"processes": {"names": process_names}}
