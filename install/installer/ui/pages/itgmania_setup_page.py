...
class ITGManiaSetupPage(BasePage):
    ...
    def __init__(self, container, app):
        ...
        self.set_title("ITGMania Setup", "Configure ITGMania integration")
    ...
    def create_widgets(self):
        ...
        self.itgmania_frame = ttk.LabelFrame(
            main_frame, text="ITGMania Configuration", padding=(10, 5)
        )
        ...
        path_label = ttk.Label(path_frame, text="ITGMania Path:", width=15)
        ...
        self.browse_button = ttk.Button(
            path_frame, text="Browse...", command=self.browse_itgmania
        )
        ...
        image_label = ttk.Label(image_frame, text="Banner Image:", width=15)
        image_label.pack(side="left", padx=(0, 5))

        self.image_var = tk.StringVar()
        self.image_entry = ttk.Entry(image_frame, textvariable=self.image_var, width=40)
        self.image_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))

        self.browse_image_button = ttk.Button(
            image_frame, text="Browse...", command=self.browse_image
        )
        ...
        self.marquee_frame = ttk.LabelFrame(
            main_frame, text="Dynamic Marquee Integration", padding=(10, 5)
        )
    ...
    def browse_itgmania(self):
        ...
        if self.app.install_manager.is_windows:
            ...
            file_path = filedialog.askdirectory(
                title="Select ITGMania Directory", initialdir=initial_dir
            )
    ...
    def browse_image(self):
        ...
        image_path = filedialog.askopenfilename(
            title="Select ITGMania Banner Image", filetypes=filetypes
        )
    ...
    def validate(self):
        ...
        if not os.path.exists(itgmania_path):
            messagebox.showerror(
                "Invalid ITGMania Path", "The specified ITGMania path does not exist."
            )
            return False
        ...
        if not self.use_default_image_var.get():
            ...
            if image_path and not os.path.isfile(image_path):
                messagebox.showerror(
                    "Invalid Image", "The specified image file does not exist."
                )
                return False
            ...
            if os.path.exists(installed_games_path):
                try:
                    with open(installed_games_path, "rb") as f:
                        ...
                        if (
                            "games" in installed_games
                            and "itgmania" in installed_games["games"]
                        ):
                            ...
                            if "path" in itgmania_config:
                                ...
                                if (
                                    ...
                                    self.default_path_checkbox.pack(
                                        anchor="w", pady=(0, 5)
                                    )
                            ...
                            if "banner" in itgmania_config:
                                ...
                                default_relative_path = os.path.normpath(
                            ...
                            if "display_module_installed" in itgmania_config:
                                self.install_module_var.set(
                                    itgmania_config["display_module_installed"]
                                )
    ...
    def save_data(self):
        ...
        self.app.user_config["itgmania"] = {"enabled": self.use_itgmania_var.get()}
        ...
        if self.use_itgmania_var.get():
            ...
            if self.use_itgmania_var.get() and self.install_module_var.get():
                try:
                    ...
                    banner_path = None
                    if (
                        not self.use_default_image_var.get()
                        and self.image_var.get().strip()
                    ):
                        banner_path = self.image_var.get().strip()

                    # Call the setup function directly
                    logging.info(
                        f"Setting up ITGMania integration with path: {itgmania_path}"
                    )
                    ...
                    try:
                        ...
                        if os.path.exists(config_path):
                            ...
                            try:
                                ...
                                logging.info(
                                    "tomli_w not available, will rely on setup script"
                                )
                    except Exception as e:
                    ...
                    if not success:
                        ...
                        logging.info(
                            "ITGMania integration setup completed successfully"
                        )
                        ...
                        try:
                            ...
                            if os.path.exists(config_path):
                                ...
                                if (
                                    "dynamic_marquee" in config
                                    and "itgmania_display_file_path"
                                    in config["dynamic_marquee"]
                                ):
                                    path = config["dynamic_marquee"][
                                        "itgmania_display_file_path"
                                    ]
                                    logging.info(
