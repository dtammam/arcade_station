...
class MameGameEntry:
    ...
    def __init__(self, parent, container, app, on_delete_callback):
        ...
        name_label = ttk.Label(name_frame, text="Game Name:", width=15)
        ...
        delete_button = ttk.Button(
            name_frame, text="Delete", command=self.delete, width=10
        )
        ...
        id_label = ttk.Label(id_frame, text="Unique ID:", width=15)
        ...
        rom_label = ttk.Label(rom_frame, text="ROM Name:", width=15)
        ...
        state_label = ttk.Label(state_frame, text="Save State:", width=15)
        ...
        state_entry = ttk.Entry(state_container, textvariable=self.state_var, width=10)
        ...
        banner_label = ttk.Label(banner_frame, text="Banner Image:", width=15)
        banner_label.pack(side="left", padx=(0, 5))

        self.banner_var = tk.StringVar()
        banner_entry = ttk.Entry(banner_frame, textvariable=self.banner_var)
        banner_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))

        browse_image_button = ttk.Button(
            banner_frame, text="Browse...", command=self.browse_banner, width=10
        )
    ...
    def browse_banner(self):
        ...
        # Otherwise fall back to the current banner path or home directory
            ...
            initial_dir = (
                os.path.dirname(self.banner_var.get())
                if self.banner_var.get()
                else os.path.expanduser("~")
            )

        file_path = filedialog.askopenfilename(
            title="Select Banner Image", filetypes=filetypes, initialdir=initial_dir
        )
    ...
    def validate(self):
        ...
        name = self.name_var.get().strip()
        if not name:
            messagebox.showerror("Invalid Name", "Please enter a name for the game.")
            return False
        ...
        if not self.rom_var.get().strip():
            messagebox.showerror(
                "Invalid ROM", "Please enter the ROM name for the game."
            )
            return False
        ...
        if not self.state_var.get().strip():
            messagebox.showerror(
                "Invalid State", "Please enter a save state for the game."
            )
            return False
        ...
        if banner_path and not os.path.isfile(banner_path):
            messagebox.showerror(
                "Invalid Banner", "The specified banner image file does not exist."
            )
            return False
...
class MAMEGamesPage(BasePage):
    ...
    def __init__(self, container, app):
        ...
        self.set_title("MAME Games Setup", "Configure MAME-based arcade games")
    ...
    def on_enter(self):
        ...
        if hasattr(self.app, "is_reconfigure_mode") and self.app.is_reconfigure_mode:
            installed_games_path = os.path.join(
                self.app.user_config.get("install_path", ""),
                "config",
                        ...
                        if "games" in installed_games:
                            ...
                            for game_id, game_info in installed_games["games"].items():
                                if "rom" in game_info:  # This is a MAME game
                                    ...
                                    entry.name_var.set(
                                        game_info.get("display_name", game_id)
                                    )
    ...
    def create_widgets(self):
        ...
        mame_config_frame = ttk.LabelFrame(
            main_frame, text="MAME Configuration", padding=(10, 5)
        )
        ...
        path_label = ttk.Label(path_frame, text="MAME Path:", width=15)
        ...
        browse_button = ttk.Button(
            path_frame, text="Browse...", command=self.browse_mame
        )
        ...
        scrollbar = ttk.Scrollbar(
            scroll_container, orient="vertical", command=self.entries_canvas.yview
        )
    ...
    def browse_mame(self):
        ...
        initial_dir = (
            os.path.dirname(self.path_var.get())
        ...
        if self.app.install_manager.is_windows:
            ...
            file_path = filedialog.askdirectory(
                title="Select MAME Directory", initialdir=initial_dir
            )
    ...
    def browse_inipath(self):
        ...
        initial_dir = (
            self.inipath_var.get()
            if self.inipath_var.get()
            else os.path.expanduser("~")
        )

        dir_path = filedialog.askdirectory(
            title="Select MAME INI Directory", initialdir=initial_dir
        )
    ...
    def validate(self):
        ...
        if not os.path.exists(mame_path):
            messagebox.showerror(
                "Invalid MAME Path", "The specified MAME path does not exist."
            )
            return False
        ...
        if inipath and not os.path.exists(inipath):
            messagebox.showerror(
                "Invalid INI Path", "The specified MAME INI path does not exist."
            )
            return False
