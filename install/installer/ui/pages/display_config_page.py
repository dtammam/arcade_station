...
class DisplayConfigPage(BasePage):
    ...
    def __init__(self, container, app):
        ...
        self.set_title(
            "Display Configuration", "Configure dynamic marquee and display settings"
        )
    ...
    def create_widgets(self):
        ...
        self.marquee_settings = ttk.LabelFrame(
            main_frame, text="Marquee Settings", padding=(10, 5)
        )
        ...
        monitor_label = ttk.Label(monitor_frame, text="Marquee Monitor:", width=15)
        ...
        color_label = ttk.Label(color_frame, text="Background Color:", width=15)
        ...
        image_label = ttk.Label(image_frame, text="Default Image:", width=15)
        image_label.pack(side="left", padx=(0, 5))

        self.image_var = tk.StringVar()

        self.image_entry = ttk.Entry(image_frame, textvariable=self.image_var, width=30)
    ...
    def toggle_marquee_options(self):
        ...
        if self.app.user_config.get("install_path"):
            self.update_installed_config(
                "dynamic_marquee.enabled", self.enable_marquee_var.get()
            )
    ...
    def browse_image(self):
        ...
        image_path = filedialog.askopenfilename(
            title="Select Default Marquee Image", filetypes=filetypes
        )
    ...
    def validate(self):
        ...
        if not self.use_default_image_var.get():
            ...
            if image_path and not os.path.isfile(image_path):
                messagebox.showerror(
                    "Invalid Image", "The specified default image file does not exist."
                )
                return False
            ...
            if "display_config" in self.app.user_config:
                ...
                if "marquee" in config and "enabled" in config["marquee"]:
                    ...
                    if "use_default_image" in config["marquee"]:
                        self.use_default_image_var.set(
                            config["marquee"]["use_default_image"]
                        )

                    # Set custom image path if not using default
                    if (
                        not self.use_default_image_var.get()
                        and "default_image" in config["marquee"]
                    ):
    ...
    def save_data(self):
        ...
        self.app.user_config["use_default_marquee_image"] = (
            self.use_default_image_var.get()
        )
        ...
        if self.app.user_config.get("install_path"):
            try:
                self.update_installed_config(
                    "display.monitor_index", int(self.monitor_var.get())
                )
                ...
                if self.use_default_image_var.get():
                    ...
                    self.update_installed_config(
                        "display.default_image_path", image_path
                    )
                else:
                    self.update_installed_config(
                        "display.default_image_path", self.image_var.get()
                    )
