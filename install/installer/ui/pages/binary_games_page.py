...
class GameEntry:
    ...
    def __init__(self, parent, container, app, on_delete_callback):
        ...
        exe_label = ttk.Label(exe_frame, text="Executable:", width=15)
        exe_label.pack(side="left", padx=(0, 5))

        self.exe_var = tk.StringVar()
        exe_entry = ttk.Entry(exe_frame, textvariable=self.exe_var, width=40)
        exe_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))

        browse_button = ttk.Button(
            exe_frame, text="Browse...", command=self.browse_executable, width=10
        )
        ...
        banner_entry = ttk.Entry(banner_frame, textvariable=self.banner_var, width=40)
    ...
    def browse_executable(self):
        ...
        initial_dir = (
            os.path.dirname(self.exe_var.get())
            if self.exe_var.get()
            else os.path.expanduser("~")
        )

        file_path = filedialog.askopenfilename(
            title="Select Game Executable", filetypes=filetypes, initialdir=initial_dir
        )
    ...
    def validate(self):
        ...
        if not exe_path:
            messagebox.showerror(
                "Invalid Executable", "Please enter the path to the game executable."
            )
            return False

        if not os.path.isfile(exe_path):
            messagebox.showerror(
                "Invalid Executable", "The specified executable file does not exist."
            )
            return False
...
class BinaryGamesPage(BasePage):
    ...
    def __init__(self, container, app):
        ...
        self.set_title("Binary Games Setup", "Configure executable-based games")
