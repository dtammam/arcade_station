...
class KioskModePage(BasePage):
    ...
    def __init__(self, container, app):
        ...
        self.set_title(
            "Kiosk Mode Configuration", "Configure automatic login and startup"
        )
    ...
    def create_widgets(self):
        ...
        uac_warning_frame = ttk.LabelFrame(
            main_frame, text="🔒 UAC & Kiosk Mode Recommendation", padding=(10, 5)
        )
        ...
        self.kiosk_settings = ttk.LabelFrame(
            main_frame, text="Kiosk Mode Settings", padding=(10, 5)
        )
        ...
        username_label = ttk.Label(autologin_frame, text="Username:", width=15)
        username_label.pack(side="left", padx=(0, 5))

        self.username_var = tk.StringVar(value=os.environ.get("USERNAME", ""))
        username_entry = ttk.Entry(
            autologin_frame, textvariable=self.username_var, width=30
        )
    ...
    def validate(self):
        ...
        if not self.username_var.get().strip():
            messagebox.showerror(
                "Invalid Input", "Please enter a username for auto-login."
            )
            return False
