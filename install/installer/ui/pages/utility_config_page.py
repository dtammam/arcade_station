...
class UtilityConfigPage(BasePage):
    ...
    def __init__(self, container, app):
        ...
        self.set_title("Utilities Setup", "Configure utilities for your Arcade Station")
    ...
    def create_lights_section(self, parent):
        ...
        lights_frame = ttk.LabelFrame(
            parent, text="Lights Configuration (Windows Only)", padding=(10, 5)
        )
        ...
        reset_label = ttk.Label(reset_frame, text="Lights reset program:", width=20)
        ...
        reset_entry = ttk.Entry(
            reset_frame, textvariable=self.lights_reset_var, width=40
        )
        reset_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))

        browse_reset_button = ttk.Button(
            reset_frame, text="Browse...", command=self.browse_lights_reset
        )
        ...
        mame_label = ttk.Label(mame_frame, text="Lights MAME executable:", width=20)
        mame_label.pack(side="left", padx=(0, 5))

        self.lights_mame_var = tk.StringVar(value=self.default_lights_mame_path)
        mame_entry = ttk.Entry(mame_frame, textvariable=self.lights_mame_var, width=40)
        mame_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))

        browse_mame_button = ttk.Button(
            mame_frame, text="Browse...", command=self.browse_lights_mame
        )
    ...
    def create_streaming_section(self, parent):
        ...
        streaming_frame = ttk.LabelFrame(
            parent, text="Streaming Configuration", padding=(10, 5)
        )
        ...
        webcam_exe_label = ttk.Label(webcam_exe_frame, text="Webcam app:", width=20)
        webcam_exe_label.pack(side="left", padx=(0, 5))

        self.webcam_exe_var = tk.StringVar(
            value="C:/Program Files/Logitech/LogiCapture/bin/LogiCapture.exe"
        )
        webcam_exe_entry = ttk.Entry(
            webcam_exe_frame, textvariable=self.webcam_exe_var, width=40
        )
        webcam_exe_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))

        browse_webcam_button = ttk.Button(
            webcam_exe_frame, text="Browse...", command=self.browse_webcam_exe
        )
        ...
        obs_label = ttk.Label(obs_frame, text="OBS executable:", width=20)
        obs_label.pack(side="left", padx=(0, 5))

        self.obs_exe_var = tk.StringVar(
            value="C:/Program Files/obs-studio/bin/64bit/obs64.exe"
        )
        obs_entry = ttk.Entry(obs_frame, textvariable=self.obs_exe_var, width=40)
        obs_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))

        browse_obs_button = ttk.Button(
            obs_frame, text="Browse...", command=self.browse_obs_exe
        )
        ...
        args_label = ttk.Label(args_frame, text="OBS arguments:", width=20)
        args_label.pack(side="left", padx=(0, 5))

        self.obs_args_var = tk.StringVar(
            value="--startstreaming --disable-shutdown-check"
        )
        args_entry = ttk.Entry(args_frame, textvariable=self.obs_args_var, width=40)
    ...
    def create_vpn_section(self, parent):
        ...
        vpn_frame = ttk.LabelFrame(
            parent, text="VPN Configuration (Windows Only)", padding=(10, 5)
        )
        ...
        vpn_dir_label = ttk.Label(
            vpn_dir_frame, text="VPN install directory:", width=20
        )
        ...
        vpn_dir_entry = ttk.Entry(
            vpn_dir_frame, textvariable=self.vpn_dir_var, width=40
        )
        vpn_dir_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))

        browse_vpn_dir_button = ttk.Button(
            vpn_dir_frame, text="Browse...", command=self.browse_vpn_dir
        )
        ...
        vpn_app_label = ttk.Label(vpn_app_frame, text="VPN application:", width=20)
        vpn_app_label.pack(side="left", padx=(0, 5))

        self.vpn_app_var = tk.StringVar(value="openvpn-gui.exe")
        vpn_app_entry = ttk.Entry(
            vpn_app_frame, textvariable=self.vpn_app_var, width=40
        )
        ...
        vpn_config_label = ttk.Label(
            vpn_config_frame, text="VPN config file:", width=20
        )
        ...
        vpn_config_entry = ttk.Entry(
            vpn_config_frame, textvariable=self.vpn_config_var, width=40
        )
        vpn_config_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))

        browse_vpn_config_button = ttk.Button(
            vpn_config_frame, text="Browse...", command=self.browse_vpn_config
        )
        ...
        vpn_wait_label = ttk.Label(vpn_wait_frame, text="Seconds to wait:", width=20)
        vpn_wait_label.pack(side="left", padx=(0, 5))

        self.vpn_wait_var = tk.IntVar(value=10)
        vpn_wait_spinbox = ttk.Spinbox(
            vpn_wait_frame, from_=1, to=60, textvariable=self.vpn_wait_var, width=5
        )
    ...
    def create_osd_section(self, parent):
        ...
        enable_osd = ttk.Checkbutton(
            osd_frame, text=osd_text, variable=self.enable_osd_var
        )
    ...
    def create_screenshot_section(self, parent):
        ...
        screenshot_frame = ttk.LabelFrame(
            parent, text="Screenshot Configuration", padding=(10, 5)
        )
        ...
        monitor_label = ttk.Label(monitor_frame, text="Screenshot Monitor:", width=20)
        ...
        sound_label = ttk.Label(sound_frame, text="Sound File:", width=20)
        sound_label.pack(side="left", padx=(0, 5))

        self.sound_file_var = tk.StringVar(
            value="../../../../assets/sounds/megatouch_yahoo.wav"
        )
        self.sound_entry = ttk.Entry(
            sound_frame, textvariable=self.sound_file_var, width=40, state="disabled"
        )
    ...
    def browse_vpn_dir(self):
        ...
        dir_path = filedialog.askdirectory(title="Select VPN Installation Directory")
    ...
    def browse_screenshot_location(self):
        ...
        dir_path = filedialog.askdirectory(title="Select Screenshot Location")
    ...
    def show_monitor_numbers(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Error", f"Could not display monitor numbers: {str(e)}"
            )
    ...
    def validate(self):
        ...
        if self.is_windows and self.enable_vpn_var.get():
            ...
            if not self.vpn_process_var.get().strip():
                messagebox.showerror(
                    "Invalid VPN Process", "Please specify the VPN process name."
                )
                return False
            ...
            if not self.vpn_config_var.get().strip():
                messagebox.showerror(
                    "Invalid VPN Config", "Please specify the VPN configuration file."
                )
                return False
        ...
        if self.enable_screenshot_var.get():
            ...
            try:
                ...
                messagebox.showerror(
                    "Invalid Monitor", "Please select a valid monitor number."
                )
                return False
    ...
    def save_data(self):
        ...
        if self.is_windows:
            utilities_config["lights"] = {
                ...
                "light_reset_executable_path": (
                    self.lights_reset_var.get() if self.enable_lights_var.get() else ""
                ),
                "light_mame_executable_path": (
                    self.lights_mame_var.get() if self.enable_lights_var.get() else ""
                ),
            }
        ...
        utilities_config["streaming"] = {
            "webcam_management_enabled": (
                self.enable_webcam_var.get()
                if self.enable_streaming_var.get()
                else False
            ),
            "webcam_management_executable": (
                self.webcam_exe_var.get()
                if self.enable_streaming_var.get() and self.enable_webcam_var.get()
                else ""
            ),
            "obs_executable": (
                self.obs_exe_var.get() if self.enable_streaming_var.get() else ""
            ),
            "obs_arguments": (
                self.obs_args_var.get() if self.enable_streaming_var.get() else ""
            ),
        }
        ...
        if self.is_windows:
            utilities_config["vpn"] = {
                ...
                "vpn_application_directory": (
                    self.vpn_dir_var.get() if self.enable_vpn_var.get() else ""
                ),
                "vpn_application": (
                    self.vpn_app_var.get() if self.enable_vpn_var.get() else ""
                ),
                "vpn_process": (
                    self.vpn_process_var.get() if self.enable_vpn_var.get() else ""
                ),
                "vpn_config_profile": (
                    self.vpn_config_var.get() if self.enable_vpn_var.get() else ""
                ),
        ...
        self.app.user_config["screenshot"] = {
            ...
            "file_location": (
                self.screenshot_location_var.get().replace("\\", "/")
                if self.enable_screenshot_var.get()
                else ""
            ),
            "monitor_index": int(
                self.screenshot_monitor_var.get()
            ),  # Keep as 0-based index
            "sound_file": (
                self.sound_file_var.get().replace("\\", "/")
                if self.enable_screenshot_var.get()
                else ""
            ),
        }
