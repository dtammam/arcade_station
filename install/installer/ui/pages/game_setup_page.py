...
class GameSetupPage(BasePage):
    ...
    def __init__(self, container, app):
        ...
        self.icon_paths = {
            "itgmania": os.path.join(
                project_root, "assets", "images", "icons", "itgmania_logo.png"
            ),
            "binary": os.path.join(
                project_root, "assets", "images", "icons", "binary_logo.png"
            ),
            "mame": os.path.join(
                project_root, "assets", "images", "icons", "mame_logo.png"
            ),
        }

        # Now call parent's __init__
        super().__init__(container, app)
        self.set_title("Game Setup", "Configure your games")
        ...
        if hasattr(self.app, "is_reconfigure_mode") and self.app.is_reconfigure_mode:
            ...
            if os.path.exists(installed_games_path):
                try:
                    ...
                    logging.warning(
                        f"Failed to load existing installed_games.toml: {e}"
                    )
    ...
    def create_widgets(self):
        ...
        # Game types frame
        types_frame = ttk.LabelFrame(main_frame, text="Game Types", padding=(10, 5))
        ...
        itg_icon = self.create_image_label(
            itg_frame, self.icon_paths["itgmania"], size=(32, 32)
        )
        ...
        binary_icon = self.create_image_label(
            binary_frame, self.icon_paths["binary"], size=(32, 32)
        )
        ...
        mame_icon = self.create_image_label(
            mame_frame, self.icon_paths["mame"], size=(32, 32)
        )
        ...
        process_frame = ttk.LabelFrame(
            main_frame, text="Setup Process", padding=(10, 5)
        )
