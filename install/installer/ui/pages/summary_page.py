...
class SummaryPage(BasePage):
    ...
    def __init__(self, container, app):
        ...
        self.set_title(
            "Installation Summary", "Review your configuration before installation"
        )
    ...
    def create_widgets(self):
        ...
        summary_frame = ttk.LabelFrame(
            main_frame, text="Configuration Summary", padding=(10, 5)
        )
        ...
        main_frame.bind(
            "<Configure>", lambda e: self.canvas.configure(width=e.width - 50)
        )
    ...
    def update_summary_text(self):
        ...
        self.summary_text.insert(
            "end", f"{self.app.user_config.get('install_path', 'Not specified')}\n\n"
        )
        ...
        if self.app.install_manager.files_copied:
            self.summary_text.insert(
                "end", "Files have been copied to the installation location.\n"
            )
            self.summary_text.insert(
                "end", "Configuration files are being updated as you make changes.\n\n"
            )
        else:
            self.summary_text.insert(
                "end", "Files will be copied when you click 'Install'.\n\n"
            )
        ...
        if self.app.user_config.get("use_dynamic_marquee", False):
            self.summary_text.insert(
                "end",
                f"Marquee Monitor: {self.app.user_config.get('marquee_monitor', 1)}\n",
            )
            self.summary_text.insert(
                "end",
        ...
        self.summary_text.insert(
            "end", f"Kiosk Mode: {'Enabled' if kiosk_mode else 'Disabled'}\n"
        )
        ...
        if process_management:
            ...
            for process_name, process_info in process_management.items():
                self.summary_text.insert(
                    "end", f"- {process_name}: {process_info.get('path', 'No path')}\n"
                )
        ...
        if vpn_config.get("enabled", False):
            ...
            self.summary_text.insert(
                "end",
                f"VPN Process: {vpn_config.get('vpn_process', 'Not specified')}\n",
            )
            self.summary_text.insert(
                "end",
        ...
        if screenshot_config.get("enabled", False):
            ...
            self.summary_text.insert(
                "end",
                f"Sound File: {screenshot_config.get('sound_file', 'Not specified')}\n",
            )
    ...
    def install(self):
        ...
        def install_thread():
            success = self.app.install_manager.perform_installation(
                self.app.user_config
            )

            # Update the UI on the main thread
            self.app.root.after(
                0, lambda: self.installation_complete(success, progress_window)
            )
    ...
    def export_config(self):
        ...
        try:
            ...
            messagebox.showinfo(
                "Export Successful", f"Configuration exported to {save_path}"
            )
        except Exception as e:
            messagebox.showerror(
                "Export Failed", f"Failed to export configuration: {str(e)}"
            )
