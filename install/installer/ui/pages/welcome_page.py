...
class WelcomePage(BasePage):
    ...
    def __init__(self, container, app):
        ...
        self.set_title(
            "Welcome to arcade_station", f"Installation for {get_platform_name()}"
        )
    ...
    def create_widgets(self):
        ...
        try:
            ...
            logo_label = ttk.Label(
                welcome_frame, text="ARCADE STATION", font=("Segoe UI", 24, "bold")
            )
            logo_label.pack(pady=20)
        ...
        self.status_label = ttk.Label(
            self.status_frame, wraplength=500, justify="center"
        )
        ...
        image_path = os.path.join(
            project_root, "assets", "images", "photos", "install_wizard.png"
        )
        if os.path.exists(image_path):
            image_label = self.create_image_label(
                welcome_frame, image_path, size=(400, 300)
            )
        ...
        self.install_options_frame = ttk.LabelFrame(
            self.status_frame, text="Installation Options", padding=10
        )
