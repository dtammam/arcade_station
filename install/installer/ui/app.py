...
class InstallerApp:
    ...
    def decide_next_page_flow(self):
        ...
        new_page_flow.append(
            self.pages[0] if len(self.pages) > 0 else WelcomePage(self.content, self)
        )
        new_page_flow.append(
            self.pages[1]
            if len(self.pages) > 1
            else InstallLocationPage(self.content, self)
        )
        ...
        if not self.is_reconfigure_mode or self.user_config.get(
            "reconfigure_games", True
        ):
            # Game setup pages
    ...
    def finish_installation(self):
        ...
        try:
            ...
            self.root.config(cursor="")
            logging.error(
                f"Unexpected error during installation: {str(e)}", exc_info=True
            )
    ...
    def check_installation_status(self):
        ...
        if "install_path" in self.user_config and self.user_config["install_path"]:
            ...
            self.is_installed = self.install_manager.check_if_installed_at_path(
                install_path
            )
