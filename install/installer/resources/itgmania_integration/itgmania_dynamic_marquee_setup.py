...
try:
    ...
    fallback_log_message(
        "Warning: Could not import arcade_station modules. Using fallback functions.",
        "SETUP",
    )
...
def copy_shim_files(itgmania_path, custom_banner_path=None):
    ...
    try:
        ...
        if source_png.exists():
            ...
            log_message(
                f"Created config file with banner path at {config_path}", "SETUP"
            )
...
def determine_log_file_path(itgmania_path, dest_file=None, is_portable=False):
    ...
    if itgmania_path.is_file():
        log_message(
            f"Warning: Log path determination was given an executable: {itgmania_path}",
            "SETUP",
        )
    ...
    if dest_file:
        ...
        log_message(
            f"Using log file path based on module location: {log_file_path}", "SETUP"
        )
    else:
        ...
        if os.path.exists(config_path):
            ...
            with open(config_path, "rb") as f:
                try:
                    ...
                    if (
                        "dynamic_marquee" in updated_config
                        and "itgmania_display_file_path"
                        in updated_config["dynamic_marquee"]
                    ):
                        actual_path = updated_config["dynamic_marquee"][
                            "itgmania_display_file_path"
                        ]
                        log_message(
                            f"Verified log path in config: {actual_path}", "SETUP"
                        )
...
def find_correct_itgmania_path(input_path):
    ...
    for root, dirs, files in os.walk(current_dir):
        if "Themes" in dirs:
            ...
            if themes_dir.exists():
                log_message(
                    f"Found Themes directory in subdirectory: {Path(root)}", "SETUP"
                )
                return Path(root)
    ...
    log_message(
        f"Could not find Themes directory, using original path: {current_dir}", "SETUP"
    )
    return current_dir
...
def setup_itgmania_integration(itgmania_path, banner_image_path=None):
    ...
    try:
        ...
        success, (dest_file, dest_image, _) = copy_shim_files(
            itgmania_base_path, banner_image_path
        )
        if not success:
            return False
        ...
        log_message("==================================================", "SETUP")
        log_message(
            "ITGMania is now configured to work with Arcade Station's dynamic marquee.",
            "SETUP",
        )
