...
class InstallationManager:
    ...
    def get_suggested_install_path(self) -> str:
        ...
        if self.is_windows:
            ...
            return os.path.join(
                os.path.expanduser("~"), "Applications", "Arcade Station"
            )
        else:
            return os.path.join(os.path.expanduser("~"), "arcade_station")
    ...
    def perform_installation(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Arcade Station installation completed successfully to {install_path}"
            )
            return True
                ...
                if item.is_dir():
                    ...
                    def copy_dir_recursive(src, dst):
                        ...
                        if os.path.isdir(src):
                            ...
                            self.logger.info(
                                f"Copying directory: {os.path.basename(src)}"
                            )
                    ...
                    try:
                        if item.is_dir():
                            ...
                            try:
                                ...
                                self.logger.warning(
                                    f"Skipping locked file: {item.name}"
                                )
                    except Exception as e:
    ...
    def _generate_config_files(self, config: Dict[str, Any], config_dir: str) -> None:
        ...
        default_config = {
            "logging": {"logdirectory": log_dir},
            "paths": {"pegasus_base_path": "../../../pegasus-fe"},
        }
        self._write_toml(
            os.path.join(config_dir, "default_config.toml"), default_config
        )
        ...
        display_config = {
            ...
            "dynamic_marquee": {
                ...
                "itgmania_display_enabled": config.get(
                    "enable_itgmania_display", False
                ),
        ...
        if config.get("itgmania", {}).get("enabled", False):
            ...
            display_config["dynamic_marquee"]["itgmania_base_path"] = config[
                "itgmania"
            ]["path"]

            # Set banner path based on whether using default or custom
            if config["itgmania"].get("use_default_image", True):
                display_config["dynamic_marquee"]["itgmania_banner_path"] = (
                    os.path.join(
                        config["install_path"],
                        "assets",
                        "images",
                ...
                display_config["dynamic_marquee"]["itgmania_banner_path"] = config[
                    "itgmania"
                ]["custom_image"]

        self._write_toml(
            os.path.join(config_dir, "display_config.toml"), display_config
        )

        # Generate installed_games.toml
        self._write_toml(
            os.path.join(config_dir, "installed_games.toml"), installed_games
        )
        ...
        processes_to_kill = {
            "processes": {
                "names": config.get("processes_to_kill", {})
                .get("processes", {})
                .get(
                    "names",
                    [
        ...
        self._write_toml(
            os.path.join(config_dir, "processes_to_kill.toml"), processes_to_kill
        )
        ...
        screenshot_config = {
            "screenshot": {
                ...
                "file_location": config.get("screenshot", {}).get(
                    "file_location", os.path.join(config["install_path"], "screenshots")
                ),
                "file_name": "",
                "quality": "High",
                "sound_file": config.get("screenshot", {}).get(
                    "sound_file",
                    os.path.join(
                        config["install_path"],
                        "assets",
        ...
        self._write_toml(
            os.path.join(config_dir, "screenshot_config.toml"), screenshot_config
        )
        ...
        if "utilities" in config:
            ...
            utility_config = {
                ...
                "osd": {
                    "enabled": config.get("enable_volume_control", False)
                    and self.is_windows,
                    "sound_osd_executable": os.path.join(
                        config["install_path"],
                        "bin",
                        "windows",
                        "AudioSwitch",
        ...
        self._write_toml(
            os.path.join(config_dir, "utility_config.toml"), utility_config
        )
        ...
        self._write_toml(
            os.path.join(config_dir, "pegasus_binaries.toml"), pegasus_binaries
        )

    def _generate_pegasus_metadata(
        self, config: Dict[str, Any], metadata_dir: str
    ) -> None:
        ...
        if config.get("itgmania", {}).get("enabled", False):
            ...
            if itgmania_path:
                ...
                python_path = os.path.join(
                    ...
                    ".venv",
                    "Scripts",
                    "pythonw.exe" if self.is_windows else "python",
                )
...
""".format(
    ...
    def _setup_windows_specific(
        self, config: Dict[str, Any], install_path: str
    ) -> None:
        ...
        if config.get("enable_kiosk_mode"):
            ...
            self._setup_windows_autologon(
                config.get("kiosk_username", ""), config.get("kiosk_password", "")
            )
    ...
    def _setup_audioswitch_settings(self, install_path: str) -> None:
        ...
        try:
            ...
            settings_xml_source = os.path.join(
                resources_dir, "audioswitch_settings", "Settings.xml"
            )

            # If not found in resources, look in the bin directory
            if not os.path.exists(settings_xml_source):
                self.logger.info(
                    "Settings.xml not found in resources, checking bin directory"
                )
                settings_xml_source = os.path.join(
                    install_path, "bin", "windows", "AudioSwitch", "Settings.xml"
                )
...
"""
    ...
    def _write_toml_manually(
        self, file_path: str, data: Dict[str, Any], indent: int = 0
    ) -> None:
    ...
    def _write_toml_section(
        self, file, data: Dict[str, Any], indent: int = 0, section_path: str = ""
    ) -> None:
            ...
            if isinstance(value, dict):
                ...
                if isinstance(value, str):
                    ...
                    if any(
                        ...
                        for path_key in ["path", "banner", "rom", "state"]
                    ):
                        value = value.replace("\\", "/")
    ...
    def _ensure_itgmania_integration(
        self, config: Dict[str, Any], install_path: str
    ) -> None:
        ...
        try:
            ...
            is_portable = os.path.exists(
                os.path.join(itgmania_base_path, "Portable.ini")
            )
            self.logger.info(
                f"ITGMania installation is {'portable' if is_portable else 'standard'}"
            )
            ...
            if is_portable:
                ...
                if self.is_windows:
                    appdata_dir = os.path.join(
                        os.path.expandvars("%APPDATA%"), "ITGmania"
                    )
                    log_file_path = os.path.join(
                        appdata_dir,
                        "Themes",
                    ...
                    appdata_dir = os.path.join(
                        os.path.expanduser("~/Library/Preferences"), "ITGmania"
                    )
                    log_file_path = os.path.join(
                        appdata_dir,
                        "Themes",
            ...
            display_config_path = os.path.join(
                install_path, "config", "display_config.toml"
            )
            if os.path.exists(display_config_path):
                self.logger.info(
                    f"Updating display_config.toml with log file path: {log_file_path}"
                )
                ...
                # Also set base path
                display_config["dynamic_marquee"]["itgmania_base_path"] = (
            ...
            self.logger.error(
                f"Error ensuring ITGMania integration: {e}", exc_info=True
            )
