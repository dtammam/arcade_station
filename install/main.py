...
def setup_logging():
    ...
    if existing_log:
        ...
        log_file = os.path.join(
            log_dir,
            f"arcade_station_installer_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
        )
...
def setup_styles():
    ...
    style.configure(
        "HeaderSub.TLabel", background="#3498db", foreground="white", font=("Arial", 10)
    )
...
def set_application_icon(root):
    ...
    icon_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "installer", "icon.ico"
    )
